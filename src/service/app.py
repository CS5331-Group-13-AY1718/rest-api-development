...
def users_register():
    if request.method == 'POST':
        ...
        password =  sha256_crypt.encrypt(str(paramsJSON['password']))
...
def users_authenticate():
    if request.method == 'POST':
        paramsJSON = request.get_json()
        username = paramsJSON['username']
        password =  str(paramsJSON['password'])
        ...
        if result == []:
            ...
            if sha256_crypt.verify(password, result[0][1]):
                """Authenticates user"""
                """Query to insert token to users table token column"""
                generatedToken = uuid.uuid4()
                query = "update users SET token='%s' where username='%s'and password='%s'" % (str(generatedToken), username,result[0][1])
                result = query_db(query)
